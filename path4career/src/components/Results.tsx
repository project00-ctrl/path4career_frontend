    ...
    <div className="min-h-screen">
      ...
      <div className="border-b border-white/5">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          <button
            onClick={onReset}
            className="flex items-center gap-2 text-gray-400 hover:text-foreground transition-colors cursor-pointer"
          >
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
            ...
            <div className="flex-1 text-center md:text-left">
              <div className="flex items-center justify-center md:justify-start gap-3 mb-3">
                <div className={`p-2 rounded-xl bg-gradient-to-r ${rc.gradient}`}>
                  <RiskIcon className="w-6 h-6 text-foreground" />
                </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-8">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-1">
                        {match.keywords.map((kw, i) => (
                          <span
                            key={i}
                            className="px-2 py-0.5 rounded-full text-[11px] bg-secondary/15 text-secondary border border-secondary/20"
                          >
                            {kw}
                          </span>
                        ))}
          ...
          <div className="bg-white/[0.03] border border-white/5 rounded-2xl p-6">
            <div className="flex items-center gap-3 mb-5">
              ...
              <h3 className="text-lg font-semibold text-foreground">Risk Areas</h3>
            </div>
            {risks.length > 0 ? (
              <div className="space-y-3">
                {risks.map((match, idx) => (
              ...
              <div className="mt-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {missingSkills.map((skill, idx) => (
                    <span
                      key={idx}
                      className="px-3 py-1 rounded-full text-xs bg-muted/50 border border-border text-gray-400 border border-white/10"
                    >
                      + {skill}
                    </span>
                  ))}
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white/[0.03] border border-white/5 rounded-2xl p-6">
            <div className="flex items-center gap-3 mb-5">
              ...
              <h3 className="text-lg font-semibold text-foreground">AI Career Suggestions</h3>
            </div>
            <div className="space-y-3">
          ...
          <div className="bg-white/[0.03] border border-white/5 rounded-2xl p-6">
            <div className="flex items-center gap-3 mb-5">
              ...
              <h3 className="text-lg font-semibold text-foreground">Resume Insights</h3>
            </div>
            <div className="space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-sm font-medium text-foreground">{atsScore}/10</span>
                </div>
                <div className="h-2 bg-muted/50 border border-border rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-blue-500 to-cyan-400 rounded-full transition-all duration-1000 ease-out"
                    style={{ width: mounted ? `${(atsScore / 10) * 100}%` : '0%' }}
                  />
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-sm font-medium text-foreground">{keywordDensity}%</span>
                </div>
                <div className="h-2 bg-muted/50 border border-border rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-purple-500 to-pink-400 rounded-full transition-all duration-1000 ease-out delay-150"
                    style={{ width: mounted ? `${keywordDensity}%` : '0%' }}
                  />
                </div>
              </div>
        ...
        <div className="bg-white/[0.03] border border-white/5 rounded-2xl p-6 mb-8">
          <div className="flex items-center gap-3 mb-5">
            ...
            <h3 className="text-lg font-semibold text-foreground">Personalized Learning Path</h3>
          </div>
            ...
            {learningPath.map((item, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-sm text-gray-300 group-hover:text-foreground transition-colors">
                    {item.title}
                  </p>
                </div>
              );
        ...
        <div className="text-center pb-12">
          <div className="inline-flex flex-col sm:flex-row gap-4">
            <button
              onClick={onReset}
              className="px-8 py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 text-primary-foreground font-semibold hover:opacity-90 transition-opacity cursor-pointer"
            >
              Analyze Another Resume
            </button>
            <button
              onClick={() => window.print()}
              className="px-8 py-3 rounded-xl bg-muted/50 border border-border border border-white/10 text-gray-300 font-medium hover:bg-muted/50 border border-border transition-colors cursor-pointer"
            >
              Download Report
            </button>
          </div>
